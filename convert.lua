...
-- <3 https://stackoverflow.com/a/42062321
print_table = function(node)
...
iprint = function(...)
    ...
    for _, arg in ipairs(args) do 
        if (type(arg) == "table") then 
            table.insert(output, "object " .. print_table(arg));
        else 
...
local functionsToListenTo = {
    ...
    "client_export", "client_exports",

    "ui_page", "before_level_meta", "after_level_meta", 
    "replace_level_meta", "data_file", "data_file", 
    "this_is_a_map", "loadscreen", "file", 
...
function createConverter(prop)
    ...
    return (function(initialData)
        if (not manifestData[prop]) then 
            manifestData[prop] = {};
        end

        local nestedFuncCall;
        local manifestDataIndex = #manifestData[prop] + 1;

        manifestData[prop][manifestDataIndex] = {};
        table.insert(manifestData[prop][manifestDataIndex], initialData);

        nestedFuncCall = function(additionalData)
            table.insert(manifestData[prop][manifestDataIndex], additionalData);
...
for _, prop in ipairs(functionsToListenTo) do 
...
iprint(json.encode(manifestData))
-- iprint(manifestData);
