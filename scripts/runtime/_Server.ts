...
export function useServer(settings: typeof defaultServerSettings = defaultServerSettings) {
    ...
    const stopResources = async (resource: string[]) => {
        const response = await fetch("http://localhost:30120/hmr/stopResources", {
            method: "POST",
            headers: {
